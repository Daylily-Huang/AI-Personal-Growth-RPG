...
2. **Never render Knowledge with Skill Mastery**: Knowledge nodes display Authority State and Epistemic Confidence, never M0–M10 mastery badges or XP bars as Knowledge properties.
   - A Knowledge Inspector **may** render an explicitly isolated, read-only **Linked Skill Summary** when the node has a real `skillId`. That subsection is typed and labelled as Skill; its Mastery, XP, and Skill Mastery Confidence come only from the Skill read model. It never converts Knowledge authority or Knowledge Epistemic Confidence into Skill state, never fabricates fallback M0/XP/confidence/level values, shows `关联技能状态暂不可用` when the Skill read fails, and exposes no mutation path. The Knowledge Node itself still does not own, derive, or display Skill Mastery/XP.
