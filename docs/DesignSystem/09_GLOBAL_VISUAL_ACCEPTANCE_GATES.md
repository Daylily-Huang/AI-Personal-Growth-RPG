...
| **Gate V6** | **Responsive Viewport Model** | Mobile-first breakpoint behavior locked (Base $< \text{md}$, `md` $\ge 48\text{rem}$, `lg` $\ge 64\text{rem}$, `xl` $\ge 90\text{rem}$); mobile bottom bar, full-screen drawer sheet, and graph LOD rendering codified. | `[x]` SPECIFIED — REVIEW PENDING |
...
| **Gate V8** | **Layered Migration Plan** | 7-phase implementation roadmap defined; Tailwind v4 `@theme` architecture specified; frontend allowlist and strict backend denylist (`src/app/api/**`, `supabase/migrations/**`, `src/lib/store/**`, `src/lib/growth-engine/**`, `0041`, `0042`) codified. | `[x]` SPECIFIED — REVIEW PENDING |
...
5. **PROHIBITION 5: Backend & API Denylist Mutation**: No changes may be made to `src/app/api/**`, `supabase/migrations/**`, `src/lib/store/**`, `src/lib/growth-engine/**`, `0041_artifact_management_authority.sql`, or `0042_artifact_settlement_integration.sql`.
