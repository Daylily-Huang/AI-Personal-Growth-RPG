...
- **Architectural Guard**: Promotion to `SUPPORTED` requires:
  ...
  2. At least four temporally distinct support observation calendar dates ($\ge 4$ distinct dates).
  3. Context from at least one completed Season ($\ge 1$ completed Season).
  4. At least two verified Growth Core outcome links ($\ge 2$ Core links: Activities, Quest completions, or Artifacts).
  5. Positive support ratio $\ge 75\%$ ($\frac{\text{Supports}}{\text{Supports} + \text{Counters}} \ge 0.75$), yielding a derived confidence level $\ge$ `HIGH`.
  6. Explicit user confirmation via `rpc_evaluate_strategy_status` or `rpc_transition_strategy_status` (prohibition of direct AI or automated promotion).
- **Failure Mode**: Any automated transition of a strategy from `HYPOTHESIS` or `TESTING` directly to `SUPPORTED` without meeting the canonical multi-observation threshold fails closed.
...
### 5. Security, Ownership, and Cross-Tenant Boundaries
 ...
 2. **Fail-Closed Per-Table Least-Privilege RLS (P2-03)**: Row Level Security is enabled across all tables, ensuring access is strictly per-table least privilege and never weaker than `auth.uid() = user_id`. Direct write verbs (`INSERT`, `UPDATE`, `DELETE`) are denied on RPC-only tables (`reward_transactions`, `reward_redemptions`, `season_quests`, `season_reviews`, `strategy_supports`, `strategy_versions`, `milestones`, `outer_loop_proposals`, `outer_loop_audit_events`). Where direct `INSERT` or `UPDATE` is permitted (e.g. `journal_entries`, draft `seasons`, `wishes`, `strategies`), database triggers and RLS `WITH CHECK` subqueries enforce cross-entity tenant isolation and protect RPC-owned lifecycle columns.
 3. **Cross-Entity Reference Validation (P0-01)**:
    - Direct join table writes (such as `season_quests`) are strictly **DENIED** to client callers via RLS (`WITH CHECK (false)` / `USING (false)`). All link operations require transactional RPCs (`rpc_link_season_quest`, `rpc_unlink_season_quest`) asserting that BOTH entities belong to `auth.uid()`.
    - Furthermore, database triggers (`trg_enforce_season_quest_tenant_isolation`, `trg_enforce_journal_entry_tenant_isolation`, `trg_enforce_strategy_support_tenant_isolation`, `trg_enforce_strategy_version_tenant_isolation`, `trg_enforce_focus_session_tenant_isolation`, `trg_enforce_protocol_version_tenant_isolation`) assert that any referenced foreign key (`seasons`, `quests`, `activities`, `strategies`, `protocols`) has an owner matching `NEW.user_id`, failing closed with a 403 error on any mismatch.
 ...
 ## 6. Database-Enforceable Direct vs. RPC Authority Guards (P1-01, P1-03)
 ...
 ### 6.2 `seasons` (Protected Status, Lifecycle Timestamps & Deletion)
 - **Direct Mutation Restriction**: Direct client `UPDATE` is permitted ONLY for descriptive metadata (`name`, `description`, `theme_color`, `icon_key`) while in `DRAFT` status.
 - **Database Trigger Guard (`trg_enforce_season_field_authority`)**: Any direct client UPDATE attempting to change `status`, `started_at`, `ended_at`, or `abandonment_reason` raises an immediate PostgreSQL exception: `"Direct lifecycle field update prohibited; must call rpc_plan_season, rpc_activate_season, or rpc_conclude_season"`.
 ...
 ### 6.3 `season_quests` (Strict RPC-Only Join Table - P0-01)
 - **Direct Write Prohibition**: Direct client `INSERT`, `UPDATE`, and `DELETE` are strictly denied via RLS (`WITH CHECK (false)` / `USING (false)`).
 - **Sole Mutation Authority**: Linking and unlinking quests to seasons is exclusively managed via `rpc_link_season_quest` and `rpc_unlink_season_quest`. Both RPC and database trigger `trg_enforce_season_quest_tenant_isolation` assert `season.user_id = auth.uid()` AND `quest.user_id = auth.uid()`.
 
 ### 6.4 `wishes` (Protected Status Transitions)
 ...
 - **Database Trigger Guard (`trg_enforce_wish_field_authority`)**: Direct client updates attempting to transition to or from `PRIMARY`, `RESERVED`, or `REDEEMED` are rejected. All financial reservation and redemption transitions must be executed through `rpc_set_primary_wish`, `rpc_reserve_wish_credits`, `rpc_unreserve_wish_credits`, `rpc_redeem_wish`, or `rpc_refund_wish_redemption`.
 
 ### 6.5 `strategies` (Protected Confidence & Status)
 ...
 ### 6.6 Ledgers & Receipts (`reward_transactions`, `reward_redemptions`, `outer_loop_audit_events`, `milestones`)
