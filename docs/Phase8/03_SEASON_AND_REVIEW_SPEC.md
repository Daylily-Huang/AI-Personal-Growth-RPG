...
### Core Boundaries
- **A Season is NOT a Goal Hierarchy**: The Quest system (`quests`) remains the sole hierarchical goal domain (Epic, Main, Major, Side). Seasons overlay temporal focus onto Quests.
...
### 2.2 Concurrency & Single Active Invariant (O13)
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> PLANNED: Finalize plan & targets
    DRAFT --> CANCELLED: Discard draft
    PLANNED --> ACTIVE: Activate season (starts clock)
    PLANNED --> CANCELLED: Cancel planned season
    
    ACTIVE --> COMPLETED: Final review confirmed & target met
    ACTIVE --> ENDED_EARLY: Premature closure (user discretion)
    ACTIVE --> ABANDONED: Objective irrelevance / descope
...
1. **`DRAFT`**: Work-in-progress season planning. Editable without constraint. May be hard-deleted if never activated.
...
6. **`ABANDONED`**: Terminal state where life circumstances, major priority shifts, or goal irrelevance led the user to stop the season. Requires post-mortem reflection to capture learning value.
7. **`CANCELLED`**: Planned season was discarded before ever becoming active.

#### Deletion & Archival Invariant (O22):
- Any Season that has ever reached `ACTIVE` status **can never be hard-deleted** through product APIs. Its historical existence, reviews, and linked contexts are permanently preserved for longitudinal growth analysis.

---

## 3. Season-to-Quest Relationship ($N:N$) (O14, O15)
...
### 3.2 Decoupled Lifecycle Invariants
- **Season Completion Does NOT Auto-Complete Quests (O15)**: When a Season concludes, linked Quests remain in their current state. An ongoing Epic Quest may span across multiple successive Seasons.
...
## 5. Structured Review Architecture (O16)
...
### 5.3 Review Authority Boundary (O16)
...
- **Immutability**: Once a Review is finalized and confirmed by the user, it becomes immutable. Any subsequent adjustments require creating a versioned superseding review (`version`, `supersedes_id`).
...
- **O015_SEASON_DOES_NOT_COMPLETE_QUEST**: Transitioning a Season to `COMPLETED` does not alter the status of any linked `IN_PROGRESS` Quests.
