...
- **O019_WISH_REDEEM_IS_IDEMPOTENT_ATOMIC**: Concurrently submitting double-redemption requests for the same wish results in exactly one redemption and one ledger entry; the concurrent loser receives HTTP 409 Conflict (`WISH_ALREADY_REDEEMED`). Replay with the identical request idempotency key returns the existing redemption receipt (HTTP 200).
