...
erDiagram
    OUTER_LOOP_PROPOSALS {
        ...
        text review_request_idempotency_key "Nullable"
...
5. **Stage 5 (Atomic Commit & Concurrency / Replay Resolution - P1-04)**:
   - The review RPC (`rpc_review_outer_loop_proposal`) acquires an exclusive row lock:
     ```sql
     SELECT * FROM outer_loop_proposals
     WHERE id = p_proposal_id AND user_id = auth.uid()
     FOR UPDATE;
     ```
   - **Case A (Proposal is PROPOSED)**:
     Executes atomic CAS state transition:
     ```sql
