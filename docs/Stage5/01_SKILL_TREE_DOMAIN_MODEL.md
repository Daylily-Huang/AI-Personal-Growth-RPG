...
- **租户层级完整性约束**：
  - `UNIQUE (user_id, id)`
  - `FOREIGN KEY (user_id, parent_id) REFERENCES public.domains(user_id, id) ON DELETE SET NULL`（杜绝跨租户指定 parent domain）
...
- **核心原则与外键约束**：
  ...
  - **域归属外键**：`FOREIGN KEY (user_id, domain_id) REFERENCES public.domains(user_id, id) ON DELETE SET NULL`（杜绝跨租户绑定 Domain）；
...
|---|---|---|---|
| **`prerequisite`** | **前置依赖**：`Source` 是学习/解锁 `Target` 的必要前提能力。 | 1. **严格 DAG**（DB 触发器禁止任何有向环）<br>2. 禁止自环 (`source != target`)<br>3. **复合外键**：`(user_id, source_skill_id)` 与 `(user_id, target_skill_id)` 分别复合引用 `skills(user_id, id)`，数据库引擎级杜绝跨租户连线 | 决定 `Target` 的解锁状态（`locked` vs `available`/`learning`）。 |
...
1. **数据库级租户绝对隔离 (Composite FKs)**：所有 `skill_edges`、`skills` 与 `domains` 必须通过 `(user_id, foreign_id)` 复合外键相互约束，彻底杜绝跨租户篡改；
