...
1. **Strict Dependency Precedence**:
   - `Stage 6A` (Domain Model, Database Authority, Migration 0039, Provenance Immutability & Anti-Cycle DAG Invariants) $\rightarrow$
...
- `src/lib/knowledge/authority-service.ts` (Sanctioned atomic state transitions: `verifyKnowledgeNode`, `rejectKnowledgeNode`, `verifyKnowledgeEdge`, `rejectKnowledgeEdge` — generic raw client mutation of verification_status is strictly prohibited).
