...
## Stage2-B 实施结论（settle_activity 权威结算）
- 0024_settlement_authority.sql 新增 `settle_activity` SECURITY DEFINER RPC（仅 service_role）：接收 delta-based SettlementToApply（jsonb），在单个事务内完成——锁定 assessment/activity 并校验 ownership、一笔 activity 结算幂等（部分唯一索引为兜底）、权威 repetition snapshot 复核（不一致返回 repetition_conflict + actualRepetitionCount，调用方重试）、skill 按 normalized_name upsert、player/skill delta 累加 + 等级重算、mastery upgrade 写入 mastery_events、request_verification 创建/去重 pending 验证（不自动升级）、supersede 同 Activity 兄弟 pending revision、确认 assessment 与 activity。ledger 的 rules_version 取 Activity 冻结值（权威），忽略客户端载荷。
- 新增 SQL 等级曲线 `xp_threshold_for_level` / `player_level_from_xp` 与 TS `levelFromXp` 完全对齐，并用 parity 测试锁定（0..12000 采样点）。
- TS 接线：`SupabaseRepository.applySettlement` 经 admin client 调用 settle_activity 并映射 jsonb 结果；`POST /api/assessments/[id]/confirm` 从 Demo getSettlementService 切换到请求级 `getRequestRepository()` + SettlementService（Supabase 配置后走真实结算，未认证 401，沿用 fail-closed）。
- 验证：`supabase db reset` 0001..0024 成功；新增 settlement-rpc.test.ts 7 项真实 pg 测试（原子结算、幂等、repetition 冲突+重试、mastery pending 去重、双用户隔离 not_owned、并发双客户端单赢、等级曲线 parity）+ authority 新增 settle_activity EXECUTE 仅 service_role 断言；完整 suite 110 passed、harness 11 passed、tsc/lint/build 通过（lint 0 errors，`_settlement` 旧 warning 随 applySettlement 实现而消失）。
- 关键坑：node-postgres 不支持在多语句 SQL 字符串中绑定 $1 参数（cleanup 静默失败导致测试残留污染 repetition 计数），已改为逐条语句执行；测试 beforeAll 先清理测试用户（fresh slate）再插入。
- 残留/边界：Skill 别名语义去重（normalized_name 仅大小写/空白归一，语义别名如 Statistics/统计学 未解析——0019 明确留待 Skill Ontology）；skill_edges 关系表尚未建模（结算只创建二级 skill，不写边）；create_activity 参数面（p_quest_id tenant 校验、p_activity_type 移除）仍未处理；HTTP 级 wiring 集成测试仍未补（resolver 单元测试 + 路由 401 映射已覆盖 fail-closed 语义）；读取路径（dashboard/skills/ledger）仍走 Demo 直至完整 Auth UI 落地。
