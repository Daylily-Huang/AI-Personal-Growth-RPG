// src/app/api/knowledge/edges/[id]/reject/route.ts
// Stage 6B Sanctioned Edge Rejection Authority Endpoint
...
export async function POST(
  ...
  try {
    ...
    const rejectedEdge = await KnowledgeAuthorityService.rejectKnowledgeEdge(
      repo.userId,
      id,
      repo,
    );

    return NextResponse.json(rejectedEdge, { status: 200 });
  } catch (error) {
    ...
    console.error("Failed to reject knowledge edge", error);
    return NextResponse.json({ error: "Failed to reject knowledge edge" }, { status: 500 });
  }
}
